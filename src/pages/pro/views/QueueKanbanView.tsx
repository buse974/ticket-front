...
function WaitingCard({ ticket, isNext, disabled, onClick }: WaitingCardProps) {
  return (
    <button
      ...
      } hover:scale-[1.01] disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:scale-100`}
    >
      {isNext && (
        <>
...
export default function QueueKanbanView(props: QueueViewProps) {
  ...
  // Timer live : tick chaque seconde
  useEffect(() => {
    const id = setInterval(() => setNow(Date.now()), 1000);
    return () => clearInterval(id);
  }, []);

  // Fetch historique du jour, re-fetch quand les current changent
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
    ...
    // Refresh quand la composition des current change
  ...
  return (
    <div className="flex h-full flex-col gap-4">
      ...
      <div className="grid flex-1 min-h-0 grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        ...
        <section className="flex min-h-0 flex-col rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-3">
          <header className="mb-3 flex items-center justify-between">
            ...
            {currentTickets.length === 0 && hasWaiting ? (
              ...
              >
                ...
                <p className="text-xs text-white/50">
                  Cliquez pour appeler le prochain ticket
                </p>
              </button>
        ...
        <section className="flex min-h-0 flex-col rounded-2xl border border-white/5 bg-white/[0.03] p-3">
          <header className="mb-3 flex items-center justify-between">
